...
class CaffeRunner:
    ...
    def __init__(self, namedtupleConfig):
        ...
        self.work_area           = namedtupleConfig.caffe_work_area
	self.output_area         = namedtupleConfig.output_work_area
	...
	self.caffe_log           = namedtupleConfig.caffe_log
        ...
        if self.work_area not in self.namedtupleConfig.snapshot_prefix:
            self.__create_workarea(self.work_area)
            self.__create_subworkarea(self.namedtupleConfig.snapshot_prefix)	
        else:
            self.__create_subworkarea(self.namedtupleConfig.snapshot_prefix)
	...
	pieces = self.namedtupleConfig.snapshot_prefix.split('/')[:-1]
	path   = "/".join(pieces)

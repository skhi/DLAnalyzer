...
if __name__ == "__main__":
    ...
    hdf5 = HDF5(range(0,9), test_selector_config.work_area)
    ...
    for key, values in train.iteritems():
        ...
        bla = myDataLoader.get_data()
       # raw, label = ImageAugmenter(bla, datasetImages=values, augType=[ 'rotation','shift', 'flip'], rotationRange=30, shiftRange=0.2, batchSize=1).apply_augment() # , 'shift', 'flip'
        raw, label = ImageAugmenter(bla, datasetImages=values, augType=[ 'default'], rotationRange=30, shiftRange=0.2, batchSize=1).apply_augment()
...
#    train_file = hdf5.combine(key='train*mean')
    ...
    test_caffe_config = test_caffe_config._replace(test_dataset=train_file, train_dataset=train_file )
...
#    myCaffe.test(train_file_number, use_trained_weights=True)
